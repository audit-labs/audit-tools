...
def parse_args() -> argparse.Namespace:
    ...
    add_standard_flags = importlib.import_module("shared.python.cli").add_standard_flags
...
def main() -> int:
    ...
    if str(repo_root) not in sys.path:
        sys.path.insert(0, str(repo_root))
    configure_logger = importlib.import_module("shared.python.logging").configure_logger
    build_metadata = importlib.import_module("shared.python.metadata").build_metadata
    ensure_run_tree = importlib.import_module("shared.python.outputs").ensure_run_tree
