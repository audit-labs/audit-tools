...
1. **Mixed script placement patterns**: some tools have subpackages
   (`applications/github/collectors`), others are flat files.
2. **No cross-repo standard output root**: scripts write to `./output`, current
   directory, or terminal only.
3. **No common `docs/` model** for per-tool audit purpose, control mapping, or
   schema.
...
- Keep existing folders temporarily; add `tools/` gradually and migrate scripts
  in phases.
- If immediate relocation is too disruptive, keep current paths but enforce
  standards first.
